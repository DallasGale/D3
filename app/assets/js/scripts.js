...
(function ($, window, document, undefined) {
  ...
  // distanceFromSun
  var distanceFromSun = solarSystemData.map(function (data) {
    return data.distanceFromSun.data;
  });
  var maxDistanceFromSun = Math.max.apply(null, distanceFromSun);
  console.log('Maximum number from [distanceFromSun]: ' + maxDistanceFromSun);

  // lengthOfDay
  var lengthOfDay = solarSystemData.map(function (data) {
    return data.lengthOfDay.data;
  });
  var maxLengthOfDay = Math.max.apply(null, lengthOfDay);
  console.log('Maximum number from [lengthOfDay]: ' + maxLengthOfDay);

  // orbitalPeriod
  var orbitalPeriod = solarSystemData.map(function (data) {
    return data.orbitalPeriod.data;
  });
  var maxOrbitalPeriod = Math.max.apply(null, orbitalPeriod);
  console.log('Maximum number from [orbitalPeriod]: ' + maxOrbitalPeriod);

  // diameter
  var diameter = solarSystemData.map(function (data) {
    return data.diameter.data;
  });
  var maxDiameter = Math.max.apply(null, diameter);
  console.log('Maximum number from [diameter]: ' + maxDiameter);

  // numberOfMoons
  var numberOfMoons = solarSystemData.map(function (data) {
    return data.numberOfMoons.data;
  });
  var maxNumberOfMoons = Math.max.apply(null, numberOfMoons);
  ...
  var yScale = d3.scaleBand().domain(solarSystemData.map(function (d) {
    // console.log('d', d.planet);
    return d.planet;
  })).range([0, height]);

  // axis
  var svg = chart.append('svg').attr('width', width).attr('height', height).style('position', 'absolute').style('top', 0);
  ...
  function render(subject) {
    ...
    console.log('hit');
    var bars = d3.select('#chart').selectAll('li').data(solarSystemData, function (d) {
      return d.planet;
    });

    bars.enter().append('li').attr('class', 'bar').style('width', 0).style('height', function (d) {
      return yScale.bandwidth() + 'px';
    }).merge(bars).transition().style('width', function (d) {
      ...
      var display = 'Planet: ' + d.planet + ' : ' + d[subject].data;
      return display;
    });
  }
  ...
  footer.select('nav').append('button').attr('class', 'btn').text('Distance From Sun').on('click', function () {
    render('distanceFromSun');
  });

  footer.select('nav').append('button').attr('class', 'btn').text('Length of Day').on('click', function () {
    render('lengthOfDay');
  });

  footer.select('nav').append('button').attr('class', 'btn').text('Orbital Period').on('click', function () {
    render('orbitalPeriod');
  });

  footer.select('nav').append('button').attr('class', 'btn').text('Diameter').on('click', function () {
    render('diameter');
  });

  footer.select('nav').append('button').attr('class', 'btn').text('Number of Moons').on('click', function () {
    render('numberOfMoons');
  });
